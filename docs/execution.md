...
Original numbering 0–13 remains intact (14 milestones total). M0 is approved; M1–M9 are complete (verification in milestone-1.md through milestone-9.md). M10–M13 remain planned. Before each milestone inspect repository status and instructions, explain changes/dependencies, implement only its slice, verify, fix, and update docs.
